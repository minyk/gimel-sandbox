...
source /etc/profile.d/nifi_exporter.sh
...
function start_nifi_exporter {
	  $EXEC "${NIFI_EXPORTER_HOME}/${NIFI_EXPORTER_ARCHIVE} ${NIFI_EXPORTER_HOME}/config.yml"
}

function stop_nifi_exporter {
    echo "Cannot stop nifi_exporter with command. Please kill process."
}
